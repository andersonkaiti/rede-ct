...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      ...
      ref={containerRef}
    >
      <div className="mx-auto max-w-7xl px-4 md:px-8 lg:px-10"></div>

      <div ref={ref} className="relative mx-auto max-w-7xl pb-10">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex justify-start pt-6 md:gap-10 md:pt-20"
          >
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              ...
              <h3 className="hidden pl-5 text-xl font-bold text-neutral-500 md:block md:pl-20 md:text-5xl dark:text-neutral-500">
                {item.title}
              </h3>
            </div>
